...
export const portfolioRouter = createTRPCRouter({
  ...
  updateCardInformativo: protectedProcedure
    .input(
      z.object({
  ...
  createCardInformativo: protectedProcedure
    .input(
      z.object({
        name: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        const { ...data } = input;
        const updatedPoportfolio = await ctx.db.portfolio.create({
          data,
        });
        return updatedPoportfolio;
      } catch (err) {
